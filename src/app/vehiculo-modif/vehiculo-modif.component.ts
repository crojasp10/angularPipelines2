...
@Component({
  selector: 'app-vehiculo-modif',
  templateUrl: './vehiculo-modif.component.html',
  styleUrls: ['./vehiculo-modif.component.scss']
})
export class VehiculoModifComponent implements OnInit {
 ...
 botonencender(){
   this.encender= this.encender===true? false:true
...
FindElement(objeto:any){
  const buscar= this.ListaObjetos.filter(i=> i.modelo==objeto.modelo)
  console.log("el elemento es "+ buscar)
};
DeleteObject(object:vehiculo){
  
  this.ListaObjetos.splice(this.ListaObjetos.indexOf(object,1));
  console.log("Este es el valor" + object )
}


DeleteObject2(objeto:any){
  const buscar= this.ListaObjetos2.filter(i=> i.modelo==objeto.modelo)
  console.log("este es el elemento"+ objeto)
}

}


const Objetos= new VehiculoModifComponent()
Objetos.Listar;
